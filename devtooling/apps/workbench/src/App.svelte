<script lang="ts">
  ...
  function selectWarp(warp: CatalogWarp): void {
    if (!selectedMap) return;
    selectedWarp = { sourceMapName: selectedMap.name, warpId: warp.warpId };
    showExits = true;
  }

  function handleCameraChange(view: AtlasViewState): void {
    currentView = view;
    replaceUrl();
  }
</script>
  ...
  <main class="min-h-screen p-[clamp(1rem,3vw,2.5rem)]">
    ...
    <div class="grid gap-4 md:grid-cols-[minmax(14rem,18rem)_minmax(0,1fr)]">
      <aside class="grid content-start gap-4">
        <RegionPicker regions={catalog.regions} activeRegionId={activeRegion.id} onSelectRegion={selectRegion} />
