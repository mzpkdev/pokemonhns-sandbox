...
The tographer provides region selection, map and map-section search, URL-persisted
...
`build/tographer/map-catalog/`.

Tographer code lives in `src/cartographer/`, including its styled interface primitives
under `src/cartographer/ui-toolkit/`. The map search combobox and exits checkbox
wrap Ark UI; compose these local controls to keep the tographer's visual and
